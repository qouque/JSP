...
<%
	...
	String areadyMemId = utils.getCookieValue(Constans.LOGINMEMBERID);
	if(StringUtils.isNotBlank(areadyMemId)){
		mem_id = areadyMemId;
		checked = "checked";
	}
...
<form action="<%=request.getContextPath()%>/login/loginProcess.do" method="post">
	<ul>
		<li>
			...
			<label><input id="saveId" type="checkbox" name="saveId" <%= checked %>/>아이디 기억하기</label>
		</li>
		<li>
